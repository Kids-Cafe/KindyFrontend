 ...
 *
 * 서버의 STT는 이 한 가지 형식만 받습니다. 브라우저의 `MediaRecorder`는 컨테이너를
 * 고를 수 있을 뿐(webm/opus, mp4/aac …) WAV를 내주지 않으므로, 소리를 직접 받아
 * 여기서 조립합니다.
 */
 ...
 * 표본율을 16kHz로 맞춥니다.
 *
 * 내려갈 때(보통 48k → 16k)는 구간 평균을 씁니다. 그냥 3칸마다 하나씩 집으면 사람
 * 목소리보다 높은 성분이 낮은 음으로 접혀 들어와(에일리어싱) 알아듣기 어려워집니다.
 * 구간 평균은 아주 성긴 저역 통과 필터라 그 접힘을 상당히 눌러 줍니다.
 *
 * 올라갈 때(마이크가 8k 같은 경우)는 선형 보간입니다. 없던 소리를 만들어 낼 수는
 * 없지만, 적어도 서버가 기대하는 표본율은 맞춰 줍니다.
 */
 ...
 *
 * 헤더는 44바이트 표준 RIFF입니다. 서버가 확장자뿐 아니라 헤더도 보므로 길이 필드를
 * 실제 데이터 크기로 정확히 채워야 합니다.
 */
export function encodeWav(chunks: Float32Array[], sampleRate: number): Blob {
  ...
  // -1~1의 실수를 16bit 정수로. 범위를 넘는 값은 자릅니다(클리핑) — 그대로 두면
  // 감싸 돌면서 아주 큰 소리가 반대 부호의 잡음으로 바뀝니다.
