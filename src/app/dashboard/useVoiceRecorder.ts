...
import { TARGET_SAMPLE_RATE, WAV_MIME_TYPE, encodeWav, rms } from "@/app/dashboard/wav";

/**
 * 마이크 녹음입니다. 화면은 "지금 녹음 중인가", "아이가 말을 시작했나", 입력 크기만
 * 알면 됩니다.
 *
 * `MediaRecorder`를 쓰지 않습니다. 서버 STT가 **16kHz WAV 하나만** 받는데
 * `MediaRecorder`는 어느 브라우저에서도 WAV를 내주지 않습니다(Chrome은 webm/opus,
 * Safari는 mp4/aac). 그래서 소리를 원본(PCM)으로 직접 받아 `wav.ts`에서 조립합니다.
 * 마침 그 원본이 손에 있으니 **소리 크기로 말이 끝났는지**도 여기서 함께 봅니다.
 *
...
/**
 * 녹음 상한입니다. 서버 multipart 한도(10MB)에는 한참 못 미치지만, 아이가 계속 떠들어도
 * 대화가 언젠가는 굴러가게 하는 쪽이 중요합니다. 16kHz 16bit면 20초가 640KB입니다.
 */
const MAX_SECONDS = 20;

/** 말이 끊긴 뒤 이만큼 조용하면 스스로 끝냅니다. 아이는 문장 중간에도 자주 쉽니다. */
...
/**
 * 오디오 스레드에서 도는 코드입니다. 문자열인 이유는 `AudioWorklet`이 **별도 모듈
 * 파일의 URL**만 받기 때문입니다. 번들러에 파일을 하나 더 얹는 대신 Blob URL로 싣습니다.
 *
 * 하는 일은 표본을 모아 한 덩어리씩 넘기는 것뿐입니다. 128표본마다 오는 콜백을 그대로
 * 메인 스레드로 흘리면 React가 초당 125번 다시 그립니다.
 */
const CAPTURE_WORKLET = `
class KindyCapture extends AudioWorkletProcessor {
  constructor() {
    super();
    this.pending = [];
    this.filled = 0;
...
registerProcessor("kindy-capture", KindyCapture);
`;
...
export function useVoiceRecorder(options: { onStart?: () => void } = {}) {
  ...
  /** 아이가 실제로 말을 시작했는지. 화면의 안내 문구가 이걸 봅니다. */
   ...
   * 녹음을 닫고 결과를 돌려줍니다.
   *
   * `spoke`가 false면 아무 말도 없었던 것이라 `null`입니다. 다만 아이가 직접 버튼을 눌러
   * 끝냈다면, 문턱을 못 넘을 만큼 작게 말했더라도 있는 소리를 그대로 보냅니다 —
   * 여기서 버리면 아이 입장에서는 말한 게 통째로 사라진 것으로 보입니다.
   */
  ...
  /** 아이가 버튼을 눌러 직접 끝내는 길입니다. */
  const stop = useCallback(() => finish(true), [finish]);

  /**
   * 조각 하나가 도착할 때마다 부릅니다. 파형, 말의 시작/끝, 상한 — 시간과 관련된 판단은
   * 전부 여기서 **표본 수**로 셉니다. `setInterval`은 탭이 뒤로 가면 느려지지만 오디오
   * 스레드는 계속 돌기 때문에, 시계보다 표본이 정확합니다.
   */
  const consume = useCallback(
    (samples: Float32Array) => {
      ...
      // 처음 얼마간은 그 방이 얼마나 조용한지부터 재고, 그 뒤로도 조용한 순간마다
      // 아주 천천히 따라갑니다(에어컨이 도중에 켜져도 문턱이 따라 올라갑니다).
      ...
      if (loudness > threshold) {
        ...
        if (take.heard) {
          ...
          // 말이 끝났습니다. 아이가 버튼을 다시 찾을 필요 없이 여기서 보냅니다.
  ...
  /**
   * 마이크를 오디오 그래프에 붙입니다. `AudioWorklet`이 먼저입니다 — 오디오 전용 스레드에서
   * 돌기 때문에 화면이 바쁠 때도 표본을 놓치지 않습니다. 실패하면(모듈 로딩을 막는 CSP 등)
   * 낡은 `ScriptProcessorNode`로 물러섭니다. 조용히 녹음이 안 되는 것보다는 낫습니다.
   */
  const attach = useCallback(
    async (context: AudioContext, source: MediaStreamAudioSourceNode) => {
      // 출력을 어딘가에 연결해야 그래프가 돌아갑니다. 볼륨 0이라 소리는 나지 않습니다
      // (그냥 destination에 붙이면 아이의 목소리가 스피커로 되돌아 나옵니다).
      ...
      try {
        ...
        const node = new AudioWorkletNode(context, "kindy-capture");
        ...
        console.warn("[Kindy] AudioWorklet을 쓸 수 없어 예전 방식으로 녹음합니다.", cause);
      }
   ...
   *
   * 끝나는 길은 셋입니다: 아이가 말을 멈췄거나(대부분), 버튼을 다시 눌렀거나, 상한에
   * 닿았거나. 어느 쪽이든 이 약속 하나가 풀립니다. 실패하거나 아무 말도 없었으면 `null`입니다.
   */
  const start = useCallback(async (): Promise<Recording | null> => {
    ...
    try {
      stream = await navigator.mediaDevices.getUserMedia({
        audio: {
          channelCount: 1,
          // 스피커로 나간 AI 목소리가 마이크로 되돌아오는 것과 방 잡음을 브라우저가
          // 먼저 걷어 줍니다. 아래의 소리 크기 판단이 그만큼 쉬워집니다.
     ...
     * 있었습니다. 그래서 스트림을 붙여 보는 데까지가 한 묶음이고, 실패하면 기기 표본율
     * 그대로 다시 열어 `encodeWav`에서 내립니다. 소리 없이 조용히 실패하는 것보다 낫습니다.
     */
    ...
    try {
      ...
      console.warn("[Kindy] 16kHz로 열지 못해 기기 표본율로 녹음합니다.", cause);
      try {
        opened = await open();
      } catch (fallbackCause) {
        console.warn("[Kindy] 소리를 받아올 수 없어요.", fallbackCause);
        teardown();
        return null;
      }
    }
    ...
    try {
      ...
      console.warn("[Kindy] 소리를 받아올 수 없어요.", cause);
      resolveRef.current = null;
      teardown();
      return null;
    }
